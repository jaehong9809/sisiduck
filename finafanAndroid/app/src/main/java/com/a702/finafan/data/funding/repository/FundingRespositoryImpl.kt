...
class FundingRepositoryImpl @Inject constructor(
    ...
    override suspend fun getFunding(fundingId: Long, fundingTitle: String): FundingDetail {
        ...
        return if (response.code == "S0000" && response.data != null) {
            response.data.toDomain(fundingId, fundingTitle)
        } else {
    ...
    override suspend fun joinFunding(fundingId: Long) {
        val response = api.joinFunding(fundingId)

        Log.d("joinFunding: ", "${response.data}")
